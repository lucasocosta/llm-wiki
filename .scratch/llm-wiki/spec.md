...
- **A versão da Fonte usada na derivação é registrada por Página Conceitual, pela tool.** Cada página conserva a versão efetivamente lida de cada Fonte contribuinte: hash de conteúdo para arquivo e SHA do arquivo lido para código, com identificação do arquivo quando a Fonte contém vários. O Espelho de Fonte compartilhado mantém rastreabilidade, mas atualizar sua versão durante a reingestão de uma página não atualiza a versão registrada nas demais. A obsolescência compara a versão usada por aquela página com a versão atual da Fonte; reingerir uma página não torna atuais outras páginas ainda derivadas de conteúdo antigo.
